...
h=waitbarpos(0, 'inserting fcc crystals');
...
for i=1:length(fcc_o)
    ...
    if(quadfrac)
        if(fcc_o(i,1)>0) fcc_o(i,1) = fcc_o(i,1) + quadfrac * fcc_o(i,1)^2; end
        if(fcc_o(i,2)>0) fcc_o(i,2) = fcc_o(i,2) + quadfrac * fcc_o(i,2)^2;end
        if(fcc_o(i,3)>0) fcc_o(i,3) = fcc_o(i,3) + quadfrac * fcc_o(i,3)^2;end        
