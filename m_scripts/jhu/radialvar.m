function [v mean vaxis center linearpix] = radialvar(imagename, frac, ...
    ptsbefpeak, peakmult, linewidth, circlediv, center)
...
linearpix = zeros(round(maxvmag*pi*2)*linewidth ,length(vaxis));
...
for i=1:pixtocollect
    theta=theta+div;
    xpx= round(dist*cos(theta+180))+cx;
    ypx= round(dist*sin(theta+180))+cy;
