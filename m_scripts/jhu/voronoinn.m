function [atom,vertices,c,atden,nelem]=voronoinn(inputfile,cutoff,minarea)
%
...
%       three times the hard-sphere radius of the largest atom type)
...
for i=1:natoms
    atom(i).border=0;
    atom(i).id = i;
    atom(i).localid =i; %here all atoms are counted, so id = localid
    atom(i).vol=vol(i);
    atom(i).pos=pos(i,:);
    nvert=length(c{i});
...
for icell=1:ncell           % index over all cells
    ...
    while iatom~=0          % index over all atoms in this cell
        ...
        atom(iatom).neighborlistlocal_allfaces = neighborlist;
        atom(iatom).neighborlistlocal = neighborlist; %this will change below
...
% polygon in (x,y,z) into the (x,y) plane so that POLYAREA can work on it.
            ...
            if realface >2
                ...
                %plot3(p(:,1),p(:,2),p(:,3),'+');pause;
     %           size(fnorm)
     %           size(p)
                xyproj=p*null(fnorm);
                %plot(xyproj(:,1),xyproj(:,2),'o');
                k=convhull(xyproj(:,1),xyproj(:,2));
                facearea(iface)=polyarea(xyproj(k,1),xyproj(k,2));
                atom(iatom).neighbor(iface).area=facearea(iface);
                %fprintf('face area is %6.4f, ',facearea(iface))
            
            else  %less than 3 points do not make an area
                facearea(iface)=0;
