function [atom,vertices,c,atden,nelem,natomspos]=voronoinnsubset(inputfile,...
    subedge,cutoff,minarea)

%subedge is the full edge
...
display(['halfedge is ' num2str(halfedge) ' angst']);

subedgesim = subedge/halfedge %bring it down to simulation box scale
...
t=2 / (atden^(1/3));
display(['thickness to be added: ' num2str(t) ' angst']);
t=t/halfedge;
%[-subedgesim/2-t  subedgesim/2+t]

border=pos(find(pos(:,1)>= -subedgesim/2-t & pos(:,1)<= subedgesim/2+t),:);
border=border(find(border(:,2)>= -subedgesim/2-t & border(:,2)<= subedgesim/2+t),:);
border=border(find(border(:,3)>= -subedgesim/2-t & border(:,3)<= subedgesim/2+t),:);
natomsposborder = length(border)

pos=pos(find(pos(:,1)>= -subedgesim/2 & pos(:,1)<= subedgesim/2),:);
pos=pos(find(pos(:,2)>= -subedgesim/2 & pos(:,2)<= subedgesim/2),:);
pos=pos(find(pos(:,3)>= -subedgesim/2 & pos(:,3)<= subedgesim/2),:);
natomspos = length(pos(:,1:3))  

%plot3(pos(:,1),pos(:,2),pos(:,3),'o', border(:,1),border(:,2),border(:,3),'r.');
%axis equal;pause;
 
[diff,i] = setdiff(border,pos,'rows');
border = border(i,:);
...
for i=1:natomspos
    if ~isinf(v(c{i},1))
        ...
        warning(['Atom ' num2str(i) ' has invalid Voronoi cell!'])
        vol(i)=nan;
    end
end
...
for i=1:natomsposborder % count over the border atoms too
    atom(i).id = pos(i,4);
    atom(i).localid = i;
    atom(i).type = pos(i,5);
    atom(i).border=0;
    atom(i).pos=pos(i,:);
    if i>natomspos atom(i).border=1; end
    if i<=natomspos atom(i).vol=vol(i); end
    nvert=length(c{i});
    %atom(i).vertices=v(c{i},:);
...
for icell=1:ncell           % index over all cells

    iatom=head(icell);      
    %display(['now processing cell number ' num2str(icell) ' of ' num2str(ncell)]);
    
    while iatom~=0          % index over all atoms in this cell
        %atom(iatom).id=iatom;
        clear neighborlist neighborlistlocal
        clear extendednn
        counter =1;
        %iatom
        nn=0;
        for jcell=1:27      % index over all local cells
            jatom=head(cellneighbor(icell,jcell)); 
            while jatom~=0      % index over all atoms in the local cell
                ...
                neighbors = find(sharedvert(:,1)==1 & sharedvert(:,2)==1 ...
                    & sharedvert(:,3)==1); %test to see if they are neighbors
                sharedvert = sharedvert(:,1); %to fit assignment in atom.neighbor.vertices
                    
                %end SH
                    
                %original code by Todd commented out
                
                %sharedvert=ismember(c{iatom},c{jatom})
                %neighbors=find(sharedvert==1)
                
                
                if ~isempty(neighbors) & (iatom ~= jatom)  % Do we share vertices?
                    nn=nn+1;
                    neighborlist(nn)=atom(jatom).id;
                    neighborlistlocal(nn)=atom(jatom).localid;
                    atom(iatom).neighbor(nn).localid=jatom;
                    atom(iatom).neighbor(nn).id=atom(jatom).id;
        ...
        atom(iatom).neighborlist_allfaces = neighborlist;
        atom(iatom).neighborlist= neighborlist; %this will change below
        atom(iatom).neighborlistlocal_allfaces = neighborlistlocal;
        atom(iatom).neighborlistlocal= neighborlistlocal; %this will change below
        atom(iatom).nn_allfaces=nn;
        atom(iatom).nn=nn;  %this will change below
        iatom=list(iatom);
    end
waitbar(icell/ncell)
end
...
%    h = waitbar(0,'Eliminating neighbors with small shared faces');
    t = cputime;
    for iatom=1:natomspos % Loop over all atoms to calculate face areas
...
% polygon in (x,y,z) into the (x,y) plane so that POLYAREA can work on it.
            realface = size(unique(single(v(atom(iatom).neighbor(iface).vertices,:)),'rows'),1);
     ...
     %           size(p)
         ...
         %       fprintf('%6.4f, ',facearea(iface))
                ...
                facearea(iface)=0;
                atom(iatom).neighbor(iface).area=0;
            end
...
%                    iatom,iface,atom(iatom).neighbor(iface).area,totarea)
                ...
                atom(iatom).neighborlist(iface)=[];
                atom(iatom).neighborlistlocal(iface)=[];
            else
                iface=iface+1;
            end
        end
