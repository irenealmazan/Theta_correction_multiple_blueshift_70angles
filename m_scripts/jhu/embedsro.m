function embedsro(config, fccorico ,frac, numxtal, filename_xtals, ...
    filename_newcon, fwhm)
...
averad = (sum(radii)/3)*frac / halfedge;
...
if(fccorico(1)=='f') display(['fcc crystals containing 13 atoms will be inserted.']); end
if(fccorico(1)=='i') display(['icosahedral crystals containing 13 atoms will be inserted.']); end
display(['they will be about ' num2str(averad*4/10) ' nm in diameter.']);
...
for i=1:numxtal
    ...
    if(fccorico(1) == 'f')
        xtal = genfcc;
        xtal = xtal * avelattice ;
        xtal = srogauss(xtal, fwhm);
    end

    if(fccorico(1) == 'i')
        xtal = genicos;
        phi = (1+sqrt(5))/2;
        xtal = xtal * sqrt((2*averad)^2/(phi^2+1));
        xtal = srogauss(xtal,fwhm);
    end
    ...
    if x<1-2*averad & x>-1+2*averad %we are fully inside the box
        xok = find(xyz(:,1)>x-2*averad & xyz(:,1)<x+2*averad);
    elseif x> 1-2*averad %too far to the positive, need some negative x's to fill in
        diff = 2*averad - (1-x);
        xok = find((xyz(:,1)>x-2*averad & xyz(:,1)<1) | (xyz(:,1)>-1 & xyz(:,1)< -1+diff));
    elseif x< -1+2*averad % too far into the negative, need some positives to fill in
        diff = 2*averad -(x+1);
        xok = find((xyz(:,1)> -1 & xyz(:,1)< x+2*averad) |( xyz(:,1)<1 & xyz(:,1) >1-diff));
    end
    
    if y<1-2*averad & y>-1+2*averad %we are fully inside the box
        yok = find(xyz(:,2)>y-2*averad & xyz(:,2)<y+2*averad);
    elseif y> 1-2*averad %too far to the positive, need some negative x's to fill in
        diff = 2*averad - (1-y);
        yok = find((xyz(:,2)>y-2*averad & xyz(:,2)<1) | (xyz(:,2)>-1 & xyz(:,2)< -1+diff));
    elseif y< -1+2*averad % too far into the negative, need some positives to fill in
        diff = 2*averad -(y+1);
        yok = find((xyz(:,2)> -1 & xyz(:,2)< y+2*averad) |( xyz(:,2)<1 & xyz(:,2) >1-diff));
    end

    if z<1-2*averad & z>-1+2*averad %we are fully inside the box
        zok = find(xyz(:,3)>z-2*averad & xyz(:,3)<z+2*averad);
    elseif z> 1-2*averad %too far to the positive, need some negative x's to fill in
        diff = 2*averad - (1-z);
        zok = find((xyz(:,3)>z-2*averad & xyz(:,3)<1) | (xyz(:,3)>-1 & xyz(:,3)< -1+diff));
    elseif z< -1+2*averad % too far into the negative, need some positives to fill in
        diff = 2*averad -(z+1);
        zok = find((xyz(:,3)> -1 & xyz(:,3)< z+2*averad) |( xyz(:,3)<1 & xyz(:,3) >1-diff));
    end
...
function sro = srogauss(sro_o, fwhm)

for i=1:length(sro_o)
    displace = randn * fwhm/2;
    sro(i,1) = sro_o(i,1) + displace; 
    displace = randn * fwhm/2;
    sro(i,2) = sro_o(i,2) + displace; 
    displace = randn * fwhm/2;
    sro(i,3) = sro_o(i,3) + displace; 
end
